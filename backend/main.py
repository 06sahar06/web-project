...
def create_event_request(event:EventRequest, db:db_dependency ):
    ...
    eventdb =EVENTREQUEST(Content = event.Content,Organiser = event.Organiser,Date = event.Date,Flyer = str(event.Flyer)) 
...
def read_accepted_events(db:db_dependency):
    ...
    return finallist
@app.get("/event_request",response_model = List[EventResponse], tags = ['EventRequest'])
def read_accepted_events(db:db_dependency):
    # get all events
